...
private:
    ...
    // reads a single colour entry from a QSettings source
    // and sets the the palette entry at 'index' to the entry read.
    void readColorEntry(QSettings& config , int index); 
    // writes a single colour entry to a QSettings source
    void writeColorEntry(QSettings& config , const QString& colorName, const ColorEntry& entry,const RandomizationRange& range) const;
