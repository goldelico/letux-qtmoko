...
void HistoryFile::map()
{
    ...
    if ( fileMap == MAP_FAILED )
    {
            readWriteBalance = 0; 
            fileMap = 0;
            //qDebug() << k_funcinfo << ": mmap'ing history failed.  errno = " << errno;
    }
}
...
HistoryScroll* HistoryTypeBuffer::scroll(HistoryScroll *old) const
{
  if (old)
  {
    // tsdogs: removed, it causes a seg fault, let's make a copy insthead
    //HistoryScrollBuffer *oldBuffer = dynamic_cast<HistoryScrollBuffer*>(old);
    /*HistoryScrollBuffer *oldBuffer = (HistoryScrollBuffer*)(old);
    if (oldBuffer)
    {
