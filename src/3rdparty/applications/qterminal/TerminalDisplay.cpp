...
#endif
// tsdogs: this causes the back button not to work ...
...
void TerminalDisplay::showResizeNotification()
{
  if (_resizing && _terminalSizeHint)
  {
     ...
     if (!_resizeWidget)
     {
...
#ifdef QTOPIA_PHONE
        QFont f("dejavu-mono",8,QFont::Normal);
#else
	    ...
	    l->setMargin(10);
        _resizeLabel = new QLabel(("Size: XXX x XXX"), _resizeWidget);
        l->addWidget(_resizeLabel, 1, Qt::AlignCenter);
        _resizeWidget->setMinimumWidth(_resizeLabel->fontMetrics().width(("Size: XXX x XXX"))+20);
...
{
    const int preeditLength = _inputMethodData.preeditString.length();
...
void TerminalDisplay::mouseMoveEvent(QMouseEvent* ev)
{
  ...
  if (dragInfo.state == diPending) 
  {
   ...
   int distance = 5; //KGlobalSettings::dndEventDelay();
