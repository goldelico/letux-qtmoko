...
void Session::close()
{
  _autoClose = true;
  _wantedClose = true;
  if (!_shellProcess->isRunning() || !sendSignal(0))
  {
     // Forced close.
...
void Session::done(int exitStatus)
{
  ...
  if (!_wantedClose && (!_shellProcess->isRunning()))
  {
    QString message;

      message = QString("Session '%1' exited with status %2.").arg(_nameTitle, exitStatus);
  }
  emit finished(this);
}
